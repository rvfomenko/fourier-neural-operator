...
class SpectralConv2d(nn.Module):
    ...
    def projection(self, input, weights):
        ...
        return torch.einsum("bixy,ioxy->boxy", input, weights) # (b)atch, (i)nput, (o)utput, (x)-modes, (y)-modes
...
class FNO2dShared(nn.Module):
    def __init__(self, n_modes_x=12, n_modes_y=12, width=32, n_layers=4, in_channels=3):
        ...
        # One FNO block, doing SpecConv2D + Channel mixing skip
        self.block = FNOBlock2d(width, n_modes_x, n_modes_y)
    ...
    def forward(self, x):
        ...
        for i in range(self.n_layers - 1):
            x = F.gelu(self.block(x)) # (B, width, H, W)

        x = self.block(x) # No GELU after last block
...
def add_grid(a): # shape: (B, 1, H, W)
    B, _, H, W = a.shape # has only the a[x,y] channel for [B] examples
    device = a.device
